...
filename = "reports/tradeoff_studies_dvast_results.txt";
ar = (0:10:360)';
times = flipud([7 8 9 10]');
thrusts = flipud([0.05 0.1 0.25 0.5 1.0 1.5 2]');
beta = 1; % beta parameter used in the terminal cost
figure(1); hold on;
for timeidx = 1:length(times)
    ...
    for thrustidx = 1:length(thrusts)
        ...
        for idx = 1:length(ar)
            ...
            [p0,fval,exitflag,output] = fsolve(@max_dvast,...
                lam_guess, ...
                options, ...
                state_values.t0, ...
                state_values.tf, ...
            ...
            if exitflag > 0
                dv = norm(beta*X(end,7)/(X(end,7)-state_values.m_a) * ...
                    (X(end,4:6)' - state_values.vf));
